...
contract SCoinPublicSale is Ownable{
    ...
    address public oracleWrapperAddress=0xB9d7528D1030221D67D6484a6728fE45a602c45a;
    address public usdtContractAddress=0xBbf126a88DE8c993BFe67c46Bb333a2eC71bC3fF;
    address payable public adminAddress=0xE380a93Db38f46866fdf4Ca86005cb51CC259771;
    ...
    function getPurchasedTokensAmount(uint256 priceOfEthInDollar,uint256 currentTokenPrice, uint256 phase,uint typeOf) internal{
        ...
        if(listOfPhases[phase].tokenSaleLimit.sub(soldTokenInfo(phase)) >=noOfTokenAccordingToCurrentPhase){
            ...
            uint256 dollarLeft=priceOfEthInDollar.sub((priceOfCurrentLeftToken.mul(currentDecimalFactor).mul(10**8)).div(decimalFactor));
            if(dollarLeft>0){
                require(phaseCount>=phase.add(1),"Please enter less amount");
